...
export default class PerformanceScoreController {
  public async index({ request }: HttpContextContract) {
    const payload = await request.validate({
      schema: PerformanceScoreValidator.list,
    });
  ...
  public async byUser({ params, response }: HttpContextContract) {
    ...
    if (scores.length === 0) {
      return response.notFound({
        message: "Aucun score trouvé pour cet employé.",
      });
    }
  ...
  public async store({ request }: HttpContextContract) {
    const payload = await request.validate({
      schema: PerformanceScoreValidator.create,
    });
  ...
  public async update({ params, request, response }: HttpContextContract) {
    const payload = await request.validate({
      schema: PerformanceScoreValidator.update,
    });
